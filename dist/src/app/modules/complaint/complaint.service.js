...
const createComplaint = async (userId, payload, files) => {
    ...
    if (payload.jobApplicationId) {
        ...
        if (application.candidateProfileId !==
            candidateProfile.id) {
            throw new Error("You cannot report another candidate's application");
        }
        // ============================================
        // 5. Verify company
        // ============================================
        if (application.job.companyId !==
            payload.companyId) {
            throw new Error("This application does not belong to this company");
        }
        // ============================================
        // 6. Verify job
        // ============================================
        if (payload.jobId &&
            application.jobId !==
                payload.jobId) {
            throw new Error("This application does not belong to this job");
        }
    }
    // ============================================
    // 7. Create complaint
...
const updateComplaint = async (userId, complaintId, payload, files) => {
    // ============================================
    // 1. Find candidate profile
    // ============================================
    const candidateProfile = await prisma.candidateProfile.findUnique({
    ...
    // 3. Check ownership
    // ============================================
    if (complaint.candidateProfileId !==
        candidateProfile.id) {
        throw new Error("You cannot update another candidate's complaint");
    }
    // ============================================
    // 4. Complaint must still be PENDING
    // ============================================
    if (complaint.status !== "PENDING") {
        throw new Error("Reviewed complaints cannot be updated");
    }
    // ============================================
    ...
    if (timePassed >
        ...
        throw new Error("You can only update a complaint within 5 hours of submission");
    }
    // ============================================
...
const deleteComplaint = async (userId, complaintId) => {
    ...
    if (complaint.candidateProfileId !==
        candidateProfile.id) {
        throw new Error("You cannot delete another candidate's complaint");
    }
    // ============================================
    // 4. Only pending complaints can be deleted
    // ============================================
    if (complaint.status !== "PENDING") {
        throw new Error("Reviewed complaints cannot be deleted");
    }
    // ============================================
    ...
    for (const evidence of complaint.evidence) {
        try {
            ...
            throw new Error("Failed to delete complaint evidence from Cloudinary");
        }
    }
    // ============================================
    // 6. Delete complaint
...
const getMyCompanyComplaintById = async (userId, complaintId) => {
    // ============================================
    ...
    // ============================================
    if (!complaint) {
        throw new Error("Complaint not found or does not belong to your company");
    }
    // ============================================
    // 4. Return complaint
    // ============================================
...
// Update Penalty
// ============================================
const updatePenalty = async (penaltyId, companyId, payload) => {
    // ============================================
    // 1. Find penalty
    // ============================================
    ...
    if (!penalty) {
        throw new Error("Penalty not found for this company");
    }
    // ============================================
    // 2. Update penalty
    // ============================================
...
// Delete Penalty
// ============================================
const deletePenalty = async (penaltyId, companyId) => {
    // ============================================
    // 1. Find penalty
    // ============================================
    const penalty = await prisma.penalty.findFirst({

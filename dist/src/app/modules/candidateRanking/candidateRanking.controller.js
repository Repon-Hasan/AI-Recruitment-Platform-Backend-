import { CandidateRankingService } from "./candidateRanking.service.js";
...
const getRankedApplicants = async (req, res) => {
    try {
        ...
        // ------------------------------------------
        const { minScore, minExperience, skill, location, } = req.query;
        // ------------------------------------------
        // Convert numbers
        // ------------------------------------------
        const parsedMinScore = minScore
            ? Number(minScore)
            : 0;
        const parsedMinExperience = minExperience
            ? Number(minExperience)
            : 0;
        // ------------------------------------------
        // Validate numbers
        // ------------------------------------------
        if (Number.isNaN(parsedMinScore) ||
            parsedMinScore < 0 ||
            parsedMinScore > 100) {
            return res.status(400).json({
                success: false,
        ...
        if (Number.isNaN(parsedMinExperience) ||
...
//# sourceMappingURL=candidateRanking.controller.js.map
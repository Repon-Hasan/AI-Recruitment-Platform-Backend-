...
export const analyzeSkillGap = async (userId, jobId) => {
  ...
  `;
    ...
    if (similarityValue === null || similarityValue === undefined) {
        throw new Error("Resume or job embedding not found. Generate embeddings first.");
    }
    const similarity = similarityValue;
    // Make sure the value stays between 0 and 1
    const normalizedSimilarity = Math.max(0, Math.min(1, Number(similarity)));
    const semanticMatchPercentage = Math.round(normalizedSimilarity * 100);
    // --------------------------------------------------
    // 5. Normalize candidate skills
    // --------------------------------------------------
    const candidateSkills = candidate.skills.map((skill) => skill.name.trim().toLowerCase());
    ...
    // --------------------------------------------------
    const matchedSkills = jobSkills.filter((jobSkill) => uniqueCandidateSkills.includes(jobSkill.normalizedName));
    // --------------------------------------------------
    // 8. Find missing skills
    // --------------------------------------------------
    const missingSkills = jobSkills.filter((jobSkill) => !uniqueCandidateSkills.includes(jobSkill.normalizedName));
    // --------------------------------------------------
    ...
    //
    const overallMatchPercentage = Math.round(semanticMatchPercentage * 0.6 +
    ...
    return {
        ...
        missingSkills: {
            high: highPrioritySkills.map((skill) => skill.name),
            medium: mediumPrioritySkills.map((skill) => skill.name),
            low: lowPrioritySkills.map((skill) => skill.name),
        },
        learningPath,
    };
};
//# sourceMappingURL=skillGap.service.js.map
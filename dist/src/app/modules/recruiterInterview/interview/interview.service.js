...
import { createInterviewSchema, rescheduleInterviewSchema, } from "./interview.validation.js";
/* =========================================================
   Helpers
========================================================= */
...
export async function createInterview(recruiterId, input) {
     ...
     */
    const isOwner = await recruiterOwnsApplication(recruiterId, application);
    if (!isOwner) {
    ...
    const interview = await prisma.$transaction(async (tx) => {
        ...
        await tx.notification.create({
            data: {
                ...
                message: `Your interview for "${application.job.title}" ` +
                    `has been scheduled.`,
         ...
         */
        const conversationId = await getOrCreateConversation(tx, application.id, application.candidateProfile.userId, recruiterId);
        /*
        ...
        await tx.message.create({
            data: {
                ...
                content: `Your interview has been scheduled for ` +
...
export async function getInterview(userId, applicationId) {
     ...
     * Recruiter ownership
     */
    const isRecruiter = await recruiterOwnsApplication(userId, application);
...
export async function getSingleInterview(userId, interviewId) {
    ...
    const application = interview.jobApplication;
    /*
     * Candidate
     */
    const isCandidate = application.candidateProfile.userId === userId;
...
export async function rescheduleInterview(recruiterId, interviewId, input) {
     ...
     */
    const isOwner = await recruiterOwnsApplication(recruiterId, interview.jobApplication);
    if (!isOwner) {
        throw new Error("FORBIDDEN");
    }
    /*
    ...
    const updatedInterview = await prisma.$transaction(async (tx) => {
        ...
        await tx.notification.create({
            data: {
                userId: interview.jobApplication
                    .candidateProfile.userId,
                type: "INTERVIEW_RESCHEDULED",
                channel: "IN_APP",
                status: "PENDING",
                title: "Interview rescheduled",
                message: `Your interview for "${interview.jobApplication.job.title}" ` +
         ...
         * Conversation
         */
        const conversationId = await getOrCreateConversation(tx, interview.jobApplication.id, interview.jobApplication
            .candidateProfile.userId, recruiterId);
        /*
         * Automatic message
         */
        await tx.message.create({
            data: {
                conversationId,
                senderId: recruiterId,
                content: `Your interview has been rescheduled to ` +
                    `${scheduledAt.toLocaleString()}.`,
                isAutomatic: true,
            },
        });
    ...
    const cancelledInterview = await prisma.$transaction(async (tx) => {
        ...
        await tx.message.create({
            data: {
                conversationId,
                senderId: recruiterId,
                content: `Your interview for "${interview.jobApplication.job.title}" ` +
                    `has been cancelled.`,
                isAutomatic: true,
            },
        });

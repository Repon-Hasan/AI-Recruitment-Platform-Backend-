...
export const AIRecruiterService = {
    ...
    async assistant(recruiterId, input) {
        ...
        if (input.jobId) {
            ...
            job = await this.findJobFromQuery(recruiterId, input.query);
        }
        if (!job) {
            throw new Error("Could not find a job matching the request");
        }
        // =====================================================
  ...
  `;
        ...
        // =====================================================
        const candidateIds = applications.map((application) => application.candidateProfileId);
        // =====================================================
        ...
        for (const candidate of semanticCandidates) {
            ...
            semanticScoreMap.set(candidate.candidateProfileId, Math.max(0, Math.min(1, similarity)));
        }
        // =====================================================
        ...
        for (const application of applications) {
            ...
            // Final Score
            // -------------------------------------------------
            const finalScore = semanticScore * 40 +
                skillScore * 30 +
                experienceScore * 20 +
                locationScore * 10;
            // -------------------------------------------------
        ...
        // STEP 9: Sort
        // =====================================================
        rankedCandidates.sort((a, b) => b.finalScore -
            a.finalScore);
        // =====================================================
        ...
        // =====================================================
        const context = this.buildLLMContext(job, topCandidates);
        // =====================================================
        ...
        return {
            ...
            candidates: topCandidates.map((candidate) => ({
                ...
                breakdown: {
                    semanticScore: Number((candidate.semanticScore *
                        100).toFixed(2)),
                    skillScore: Number((candidate.skillScore *
                        100).toFixed(2)),
                    experienceScore: Number((candidate.experienceScore *
                        100).toFixed(2)),
                    locationScore: Number((candidate.locationScore *
                        100).toFixed(2)),
                },
            })),
            answer,
    ...
    calculateSkillScore(job, candidate) {
         ...
         */
        const requiredSkills = job.requiredSkills?.map((jobSkill) => jobSkill.name
            .toLowerCase()
            .trim()) ?? [];
        const candidateSkills = candidate.skills?.map((skill) => skill.name
            .toLowerCase()
        ...
        for (const required of requiredSkills) {
            const isMatched = candidateSkills.some((candidateSkill) => candidateSkill === required ||
    ...
    calculateExperienceScore(job, candidate) {
        ...
        const matches = keywords.filter((keyword) => jobDescription.includes(keyword) &&
            candidateExperience.includes(keyword));
        if (matches.length > 0) {
            return 1;
        }
        return 0.5;
    ...
    calculateLocationScore(job, candidate) {
        ...
        const candidateLocation = candidate.location
            .toLowerCase()
            .trim();
        if (jobLocation.includes(candidateLocation) ||
...
//# sourceMappingURL=aiRecruiter.service.js.map
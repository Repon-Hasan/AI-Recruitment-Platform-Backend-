...
export const generateCandidateEmbedding = async (candidateProfileId) => {
    ...
    const projects = candidate.projects
        .map((project) => `${project.name}: ${project.description ?? ""} ${project.technologies ?? ""}`)
        .join("\n");
...
${resumeText}
  `.trim();
    ...
    if (embedding.length !== 2048) {
        throw new Error(`Expected 2048 dimensions but received ${embedding.length}`);
    }
    console.log("Candidate embedding dimensions:", embedding.length);
    // ==========================================
...
`;
    ...
    // 8. Return result
    // ==========================================
    console.log("Candidate embedding saved successfully:", candidateProfileId);
    return {
        candidateProfileId,
...
//# sourceMappingURL=candidate.embedding.service.js.map